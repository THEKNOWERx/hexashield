...
const AttackPathView = ({ headerTitle, headerSubtitle }) => {
  ...
  const nodesList = useMemo(() => {
    ...
    // If the backend generated an attack graph with 2 or fewer nodes, it means there are no actual vulnerable paths found (only Attacker -> Target).
    if (!data.graph || !data.graph.nodes || data.graph.nodes.length <= 2) {
      return { isSecure: true, targetName };
    }
  ...
  if (error) {
    if (error.includes("No active targets or scans found")) {
      return (
        <div className="space-y-6">
          <GlobalHeader title={headerTitle} subtitle={headerSubtitle} />
          <div className="flex flex-col items-center justify-center p-12 bg-[#020203] border border-white/10 rounded-[2rem] shadow-[0_0_40px_rgba(0,0,0,0.8)] min-h-[600px] text-center">
            <Target className="w-20 h-20 mb-6 text-gray-600" />
            <h2 className="text-2xl font-bold mb-3 text-white">No Active Target</h2>
            <p className="text-gray-400 font-mono max-w-md">There is no scan data available to generate an attack path. Please run a <span className="text-[#00c8ff]">Network Scan</span> first to populate the intelligence engine.</p>
          </div>
        </div>
      );
    }
  ...
  if (nodesList?.isSecure) {
    return (
      <div className="space-y-6">
        <GlobalHeader title={headerTitle} subtitle={headerSubtitle} />
        <div className="flex flex-col items-center justify-center p-12 bg-[#020203] border border-white/10 rounded-[2rem] shadow-[0_0_40px_rgba(0,0,0,0.8)] min-h-[600px] text-center">
          <Shield className="w-20 h-20 mb-6 text-green-500" />
          <h2 className="text-2xl font-bold mb-3 text-white">Target Secure</h2>
          <p className="text-gray-400 font-mono max-w-md">No exploitable paths or critical vulnerabilities found on <span className="text-[#00c8ff]">{nodesList.targetName}</span>. Attack path simulation cannot be generated.</p>
        </div>
      </div>
    );
  }
